...
class FolderCreateDto(BaseModel):
    ...
    name: str = Field(..., min_length=1, max_length=255, description="Name of the folder.")
    workspace_id: int = Field(..., description="ID of the workspace.")
    parent_id: int | None = Field(
        default=None, description="Parent folder ID, or None for workspace root."
    )
...
class FolderUpdateDto(BaseModel):
    ...
    name: str | None = Field(
        default=None, min_length=1, max_length=255, description="New name of the folder."
    )
    parent_id: int | None = Field(
        default=None, description="New parent folder ID (or None to move to root)."
    )
    color: str | None = Field(
        default=None, description="New hex color code."
    )
...
class MoveItemsDto(BaseModel):
    ...
    workspace_id: int = Field(..., description="Workspace ID for security check.")
