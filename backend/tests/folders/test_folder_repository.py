...
class TestFolderRepository:
    ...
    async def test_get_folder_by_id(self, folder_repo, mock_db):
        folder = Folder(id=1, workspace_id=1, user_email="a@b.com", name="Folder")
    ...
    async def test_list_by_parent(self, folder_repo, mock_db):
        ...
        mock_result.all.return_value = [
            (folder, 3, 2, 1)  # folder, media_count, asset_count, subfolder_count
        ]
        mock_db.execute.return_value = mock_result
    ...
    async def test_get_tree(self, folder_repo, mock_db):
        f1 = Folder(id=1, workspace_id=1, user_email="a@b.com", name="Root", parent_id=None)
        f2 = Folder(id=2, workspace_id=1, user_email="a@b.com", name="Child", parent_id=1)
        mock_result = MagicMock()
    ...
    async def test_move_media_items(self, folder_repo, mock_db):
        ...
        count = await folder_repo.move_media_items([1, 2], workspace_id=1, destination_folder_id=3)
        assert count == 2
        mock_db.commit.assert_called_once()
    ...
    async def test_move_source_assets(self, folder_repo, mock_db):
        ...
        count = await folder_repo.move_source_assets([10], workspace_id=1, destination_folder_id=3)
        assert count == 1
        mock_db.commit.assert_called_once()
    ...
    async def test_move_folders(self, folder_repo, mock_db):
        ...
        count = await folder_repo.move_folders([5], workspace_id=1, destination_folder_id=3)
        assert count == 1
        mock_db.commit.assert_called_once()
